        ...
        $('#procurement').DataTable({
            ...
            buttons: [
                ...
                },
                {
                    extend: 'excelHtml5',
                    pageSize: 'Legal',
                    title: "Daftar <?= ucwords($this->uri->segment(2));?> Tahun Pengadaan <?= $year;?> Pemerintah Kota Tangerang",
                    exportOptions: {
                        columns: ':visible'
                    }
                },
                {
                    ...
                    customize : function(doc) 
                    {
                        doc.content.splice(0, 1, {
                          text: [
                            {
                                text: "Daftar <?= ucwords($this->uri->segment(2));?> Tahun Pengadaan <?= $year;?> Pemerintah Kota Tangerang \n\n",
                                fontSize: 14,
                                alignment: 'center'
                            }, 
                            {
                                text: "Total <?= ucwords($this->uri->segment(2));?> Tahun Pengadaan <?= $year.' : '.$count;?> Unit\n\n\n",
                                fontSize: 9,
                                alignment: 'left'
                            }
                          ]
                        ...
                        var tr = $('#procurement tbody tr:first-child');
                        var trWidth = $(tr).width();

                        var length = $('#procurement tbody tr:first-child td').length;

                        $('#procurement').find('tbody tr:first-child td').each(function()
                        {
